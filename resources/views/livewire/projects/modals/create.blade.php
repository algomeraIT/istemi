<div class="p-10">
    <div class="w-full items-center justify-between mb-10">
    ...
    <div class="flex gap-10">
        <!-- Sidebar Tabs -->
        <div class="w-fit flex flex-col items-center h-1/2">
            <ul class="flex flex-col items-center z-10">
                @for ($i = 1; $i <= 4; $i++)
                    <div
                        class="border-l-4 border-accent-DEFAULT {{ $currentTab < $i ? 'border-dashed !border-[#B0B0B0]' : '' }} min-h-10 max-h-14">
                    </div>

                    <li>
                        <button wire:click="$set('currentTab', {{ $i }})"
                            class="size-10 flex items-center justify-center rounded-full border-4 border-[#B0B0B0] text-[#B0B0B0] font-bold
        {{ $currentTab > $i - 1 ? 'border-accent-DEFAULT text-accent-DEFAULT' : '!border-2 !size-8 text-sm font-medium' }}">
                            {{ $i }}
                        </button>
                    </li>
                @endfor
        ...
        <!-- Tab Content -->
        <div class="flex-1">
            <div class="py-5 px-10 border-1 overflow-hidden overflow-y-auto max-h-[500px] bg-[#F8FEFF]">
                @if ($currentTab === 1)
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <flux:field data-input>
                            ...
                            <flux:select variant="listbox" wire:model.live="formData.responsible_id">
                                <flux:select.option value="">Seleziona</flux:select.option>
                            ...
                            <flux:error name="formData.responsible_id" />
                        </flux:field>
                    ...
                    <div>
                        ...
                        <div class="lg:flex gap-4">
                            <flux:field data-input class="flex-1">
                                ...
                                <flux:input type="number" min="0" wire:model.live="formData.starting_price" />
                            ...
                            @foreach ($firms_and_percentage_keys as $index => $key)
                                <div class="flex gap-2 items-center mb-2">
                                    ...
                                    <flux:input class="w-10" type="number" min="0"
                                        wire:model="firms_and_percentage_values.{{ $index }}" placeholder="%" />
                        ...
                        <flux:field data-input class="flex-1 mt-10">
                            ...
                            <flux:editor class="**:data-[slot=content]:min-h-[80px]!" wire:model.live="formData.note"
                                placeholder="Notes" />
                    ...
                    <div class="">
                        ...
                        <div class="flex flex-col">
                            ...
                            <flux:field data-input>
                                ...
                                <flux:editor class="**:data-[slot=content]:min-h-[100px]!"
                                    wire:model.live="formData.project_scope" placeholder="Descrivi l'ambito..." />
                                    ...
                                    <div class="mt-4 bg-gray-50 p-2 border rounded">
                                        <div class="flex justify-between items-center">
                                            <p class="text-sm"><strong x-text="stk.name"></strong> - <span
                                                    x-text="stk.role"></span> - <span x-text="stk.email"></span></p>
                                            <button type="button" @click="remove(i)"
                                                class="text-red-500 hover:text-red-700">&times;</button>
                                        </div>
                                        <input type="hidden" :name="'formData.stackholders[' + i + '][name]'"
                                            :value="stk.name" wire:model="formData.stackholders[i].name" />
                                        <input type="hidden" :name="'formData.stackholders[' + i + '][email]'"
                                            :value="stk.email" wire:model="formData.stackholders[i].email" />
                                        <input type="hidden" :name="'formData.stackholders[' + i + '][role]'"
                                            :value="stk.role" wire:model="formData.stackholders[i].role" />
                                    </div>
                                </template>
            ...
            {{-- Action Buttons --}}
            <div class="bg-white mt-8 flex justify-between">
